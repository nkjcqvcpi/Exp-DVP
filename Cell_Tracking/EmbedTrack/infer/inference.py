...
from utils.utils import get_img_files
import matplotlib
import numpy as np
import pandas as pd
import tifffile
...
from utils.utils import get_indices_pandas
...
class InferenceDataSet(Dataset):
    ...
    def __init__(self, img_crops_curr_frame, img_crops_prev_frame, bg_id=0, transform=None):
        """
        Initialize dataset
        Args:
...
def generate_crops(image_file, crop_size=256, overlap=0.25):
    ...
    y_start = (np.arange(0, size_y - crop_size + 1, crop_size * (1 - overlap))).astype(int)
    x_start = (np.arange(0, size_x - crop_size + 1, crop_size * (1 - overlap))).astype(int)
...
def patch_crops(image_crops, position_index, padded_img_size, img_size, window, crop_size=256, overlap=0.25):
    """
    ...
    y_start = (np.arange(0, padded_img_size[1] - crop_size + 1, crop_size * (1 - overlap))).astype(int)
    x_start = (np.arange(0, padded_img_size[2] - crop_size + 1, crop_size * (1 - overlap))).astype(int)
    upper_left = list(product(y_start, x_start))
    for index, crop in zip(position_index, image_crops):
        row_start, col_start = upper_left[index]
        image_padded[:, row_start: row_start + crop_size, col_start: col_start + crop_size] += np.multiply(window, crop)
        weights[:, row_start: row_start + crop_size, col_start: col_start + crop_size] += window
    image_padded = np.divide(image_padded, weights)

    image = image_padded[:, crop_size // 2: crop_size // 2 + img_size[1], crop_size // 2: crop_size // 2 + img_size[2]]
    return image


def augment_image(img, batch=True):
    """
...
def deaugment_segmentation(segmentation_batch, batch=True):
    """
    Deaugment (rotate+flip) a batch of predictions and calculate mean prediction
    :param segmentation_batch: torch.tensor
    ...
    seg.append(rotate_segmentation_tensor(segmentation_batch[1].squeeze(0), k=-1, dims=[1, 2]))
    seg.append(rotate_segmentation_tensor(segmentation_batch[2].squeeze(0), k=-2, dims=[1, 2]))
    seg.append(rotate_segmentation_tensor(segmentation_batch[3].squeeze(0), k=-3, dims=[1, 2]))
    seg.append(flip_segmentation_tensor(segmentation_batch[4].squeeze(0), [1]))
    seg.append(
        rotate_segmentation_tensor(flip_segmentation_tensor(segmentation_batch[5].squeeze(0), [1]), k=-1, dims=[1, 2])
    )
    seg.append(
        rotate_segmentation_tensor(flip_segmentation_tensor(segmentation_batch[6].squeeze(0), [1]), k=-2, dims=[1, 2])
    )
    seg.append(
        rotate_segmentation_tensor(flip_segmentation_tensor(segmentation_batch[7].squeeze(0), [1]), k=-3, dims=[1, 2])
    )
    if batch:
        # shape: 8, c, h, w, b
        seg = torch.stack(seg)
...
def rotate_segmentation_tensor(segmentation, k, dims):
    ...
    if k % 2 and k > 0:  # if rotation of 90, 270 degree swap sigma x, sigma y as well
        ...
        other_classes = torch.cat([sigma_x[np.newaxis, ...], sigma_y[np.newaxis, ...], seed_map[np.newaxis, ...]],
                                  dim=0)
...
def deaugment_offset(offset_batch, batch=True):
    if batch:
    ...
    offset.append(rotate_offset_tensor(flip_offset_tensor(offset_batch[5].squeeze(0), [1]), k=-1, dims=[1, 2]))
    offset.append(rotate_offset_tensor(flip_offset_tensor(offset_batch[6].squeeze(0), [1]), k=-2, dims=[1, 2]))
    offset.append(rotate_offset_tensor(flip_offset_tensor(offset_batch[7].squeeze(0), [1]), k=-3, dims=[1, 2]))
    if batch:
        # shape: 8, c, h, w, b
...
def create_inference_dict(name="2d", batch_size=16, workers=0):
    """
    ...
    dataset_dict = {
        "name": name,
        "kwargs": {"transform": set_transforms},
...
def init_model(model_dict, configs):
    ...
    state = convert_state_dict(torch.load(configs["model_cktp_path"])["model_state_dict"])
...
def smooth_prediction(data_loader, model, device):
    """
    ...
    with torch.no_grad():
        for batch in data_loader:
            img_curr_aug = augment_image(batch["image_curr"], batch=True).to(device)
            img_prev_aug = augment_image(batch["image_prev"], batch=True).to(device)

            seg_images_curr_aug, seg_images_prev_aug, offsets_aug = model(img_curr_aug, img_prev_aug)

            seg_images_curr = deaugment_segmentation(seg_images_curr_aug)
            seg_images_prev = deaugment_segmentation(seg_images_prev_aug)

            offsets = deaugment_offset(offsets_aug)
...
def infer_sequence(model, data_config, model_config, config, cluster, min_mask_size):
    ...
    data_dirs = dict(tracking_dir=os.path.join(config["res_dir"], "tracking"), )
    ...
    for i, t_curr_frame in enumerate(time_points[:-1]):
        ...
        crops_curr_frame = generate_crops(os.path.join(img_file_t_curr), config["crop_size"], config["overlap"])
        crops_prev_frame = generate_crops(os.path.join(img_file_t_prev), config["crop_size"], config["overlap"], )
        data_loader = torch.utils.data.DataLoader(
            InferenceDataSet(crops_curr_frame, crops_prev_frame, **data_config["kwargs"]),
        ...
        (img_index, all_seg_images_curr, all_seg_images_prev, all_offsets) = smooth_prediction(
            data_loader, model, config.DEVICE)
        ...
        if i == 0:
            ...
            instances_curr = (
                cluster_prediction(
                    cluster,
                    torch.tensor(seg_curr, device=config.DEVICE, dtype=torch.float),
                    min_mask_size=min_mask_size,
                ).detach().cpu().numpy()
            )
            mask_idx_curr = get_indices_pandas(instances_curr)
            max_tracking_id = max(mask_idx_curr.keys())
            lineage = {m_idx: [t_curr_frame, t_curr_frame, 0] for m_idx in mask_idx_curr.keys()}
            print(f"Save tracking mask {os.path.basename(img_file_t_curr)}")
            tifffile.imwrite(os.path.join(data_dirs["tracking_dir"], os.path.basename(img_file_t_curr)), instances_curr)

        else:
            mask_idx_curr = get_indices_pandas(
                tifffile.imread(os.path.join(data_dirs["tracking_dir"], os.path.basename(img_file_t_curr)))
            )

        instances_prev = (
            cluster_prediction(
                cluster,
                torch.tensor(seg_prev, device=config.DEVICE, dtype=torch.float),
                min_mask_size=min_mask_size,
            ).detach().cpu().numpy()
        )
        ...
        for m_idx_curr, est_prev_position in estimated_prev_positions.items():
            m_idx, counts = np.unique(instances_prev[est_prev_position[0], est_prev_position[1]], return_counts=True, )
    ...
    df.to_csv(os.path.join(data_dirs["tracking_dir"], "res_track.txt"), sep=" ", index=False, header=False)


def link_masks(links_forward, mask_idx_prev, lineage, max_tracking_id, time_point, img_shape):
    # generate tracking mask
...
def calc_previous_position_estimate(curr_mask_positions, offset, grid_y, grid_x, pixel_y, pixel_x):
    """
    ...
    estimated_prev_positions = {
        m_id: _clip_positions(np.array(idx) - offset[..., idx[0], idx[1]].astype(int), img_shape)
...
def _clip_positions(estimated_position, img_shape):
    ...
    estimated_position = estimated_position[:, ~np.any(estimated_position > img_shape - 1, axis=0)]
...
def remove_single_frame_tracks(tracking_dir):
    ...
    single_frame_tracks = lineage[(lineage["t_start"] == lineage["t_end"]) & (lineage["predecessor_id"] == 0)]
    single_frame_tracks = single_frame_tracks[~np.isin(single_frame_tracks["track_id"], lineage["predecessor_id"])]
    ...
    lineage.to_csv(os.path.join(res_dir, "res_track.txt"), sep=" ", index=False, header=False)
...
def extend_grid(cluster, image_size):
    """
    Extend the grid to the image size.
    The model was trained on a small grid - now extend the grid to fit the full image size.
    Args:
        cluster: Cluster
            clustering instance
        image_size: tuple
    ...
    if image_size[1] > cluster.grid_x:
        xm = torch.linspace(0, cluster.pixel_x * (image_size[1] - 1) / (cluster.grid_x - 1), image_size[1])
    if image_size[0] > cluster.grid_y:
        ym = torch.linspace(0, cluster.pixel_y * (image_size[0] - 1) / (cluster.grid_y - 1), image_size[0])
...
def foi_correction(tracking_dir, cell_type):
    ...
    for segm_file in os.listdir(tracking_dir):
        ...
        tifffile.imsave(os.path.join(tracking_dir, segm_file), segm_mask.astype(np.uint16))
...
def remove_missing_tracks_from_lineage(tracking_dir):
    ...
    missing_masks = set(lineage_data["m_id"].values).symmetric_difference(set(tracks.keys()))
    ...
    lineage_data.to_csv(os.path.join(tracking_dir, "res_track.txt"), sep=" ", index=False, header=False)
...
def edit_tracks_with_missing_masks(tracking_dir):
    ...
    for track_id, t_tracklets in fragmented_tracks.items():
        ...
        tracklet_parent = 0
        for i_tracklet, (t_start, t_end) in enumerate(t_tracklets):
            ...
            lineage_data = lineage_data.append(
                {"m_id": max_track_id, "t_s": t_start, "t_e": t_end, "pred": tracklet_parent}, ignore_index=True
            )
...
def find_tracklets(data_path, lineage_name="res_track.txt"):
    ...
    :param data_path: string or posix path to tracking directory
    :param lineage_name:
    """
