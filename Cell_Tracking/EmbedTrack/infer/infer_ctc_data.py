...
from utils.create_dicts import create_model_dict
import tifffile
import torch
...
def inference(raw_data_path, model_path, config_file, batch_size=32):
    ...
    image_size = tifffile.imread(os.path.join(raw_data_path, os.listdir(raw_data_path)[0])).shape
    ...
    project_config = dict(
        ...
        img_size=image_size,
        padded_img_size=calc_padded_img_size(image_size, crop_size, overlap)[0]
    )
    window_function_1d = gaussian(project_config["crop_size"], project_config["crop_size"] // 4)
    project_config["window_func"] = window_function_1d.reshape(-1, 1) * window_function_1d.reshape(1, -1)

    dataset_dict = create_inference_dict(batch_size=batch_size)

    # init model
    input_channels = train_config["model_dict"]["kwargs"]["input_channels"]
    n_classes = train_config["model_dict"]["kwargs"]["n_classes"]
    model_dict = create_model_dict(input_channels=input_channels, n_classes=n_classes)
    ...
    # clustering
    cluster = Cluster(project_config["grid_y"], project_config["grid_x"],
                      project_config["pixel_y"], project_config["pixel_x"])
    cluster = extend_grid(cluster, image_size)
    tracking_dir = os.path.join(project_config["res_dir"], "tracking")
    infer_sequence(model, dataset_dict, model_dict, project_config, cluster,
    ...
    lineage = pd.read_csv(os.path.join(tracking_dir, "res_track.txt"), sep=" ", header=None)
    max_id = lineage[0].index.max()
    if max_id >= 2 ** 16 - 1:
        raise AssertionError("Max Track id > 2**16 - uint16 transformation needed for ctc "
...
def fill_empty_frames(mask_dir):
    ...
    for empty_t, empty_file in empty_time_steps:
        nearest_filled_frame = filled_files[np.argmin(abs(np.array(filled_t) - empty_t))]
        ...
        for mask_idx in mask_ids:
            lineage.loc[mask_idx]["t_start"] = min(empty_t, lineage.loc[mask_idx]["t_start"])
            lineage.loc[mask_idx]["t_end"] = max(empty_t, lineage.loc[mask_idx]["t_end"])
...
if __name__ == "__main__":
    ...
    PARSER = ArgumentParser(description="Segmentation and Tracking using EmbedTrack Model")
    ...
    inference(ARGS.raw_image_path, ARGS.model_path, ARGS.config_file_path, ARGS.batch_size)
