...
"""
import models.BranchedERFNet as BranchedERFNet
import torch
from torch import nn
from models.erfnet import Decoder
...
class TrackERFNet(BranchedERFNet):
    ...
    def __init__(self, n_classes=[4, 1, 2], input_channels=1, encoder=None):
        ...
        """
        super(TrackERFNet, self).__init__(n_classes, input_channels=input_channels, encoder=encoder)
    ...
    def forward(self, curr_frames, prev_frames):
        ...
        features_stacked = torch.cat([features_curr_frames, features_prev_frames], dim=1)  # batchsize x 2 x h x w
        segm_prediction = torch.cat([decoder.forward(features_encoder) for decoder in self.decoders[:-1]], 1)
