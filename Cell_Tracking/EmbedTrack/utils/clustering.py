...
class Cluster(nn.Module):
    def __init__(self, grid_y, grid_x, pixel_y, pixel_x):
        ...
        smoothing_filt = torch.nn.Conv2d(1, 1, (3, 3), padding="same", padding_mode="reflect")
        smoothing_filt.weight.data = torch.ones(1, 1, 3, 3) / 9
        smoothing_filt.to(device)
        self.smoothing_filt = smoothing_filt

    def cluster_pixels(self, prediction, n_sigma=2, min_obj_size=10, return_on_cpu=True):
        """
        ...
        sigma = torch.exp(torch.sigmoid(prediction[2 : 2 + n_sigma]) * 10)  # n_sigma x h x w
        ...
        selected_pixels = (seed_map > 0.5)  # use only pixels with very high seediness scores as potential seeds!
        seeds = self.find_seeds(
            spatial_emb[selected_pixels.expand_as(spatial_emb)].view(2, -1), self.smoothing_filt.weight.numel() * 0.5,
        ).expand_as(seed_map)
        ...
        # instance_map_masked = torch.zeros(height, width, device=device)
        # dist_map = torch.zeros(height, width, device=device)
        # unclustered = torch.ones(seeds.sum(), device=device)

        instance_map_masked = torch.zeros(height, width, device=device, dtype=torch.short)
        ...
        while unclustered.sum():
            ...
            if ((colliding_pixel / proposal.sum()) < 0.5) and (proposal.sum() >= min_obj_size):
    ...
    def find_seeds(self, spatial_emb_masked, min_cluster_points):
        ...
        if len(pix_positions_unique) > 0:
            clustered_pixels[pix_positions_unique[0].long(), pix_positions_unique[1].long()] = counts.float()
        with torch.no_grad():
            # multiply with filter size to get a sum filter and not an avg
            clustered_pixels_smoothed = (
                self.smoothing_filt(clustered_pixels[np.newaxis, np.newaxis, ...]).squeeze()
