...
class VisualizeTraining:
    def __init__(self, cluster, export_path, export_data=True, export_format=".pdf", grid_x=1024, grid_y=1024,
                 pixel_x=1, pixel_y=1, n_sigma=2, color_map="plasma", device='cpu'):
        """
        ...
        self.grid = calc_grid(grid_x, grid_y, pixel_x, pixel_y, device=device)
        self.grid_shape = torch.tensor(self.grid.size()[1:], device=device).view(-1, 1)
    ...
    @staticmethod
    def calc_fig_size(n_subplots, fig_size):
    ...
    def add_plot(self, n_plots, fig_name):
        ...
        fig, ax = plt.subplots(*n_plots, figsize=self.calc_fig_size(n_plots, self.subplt_size))
        ...
        with torch.no_grad():
            ...
            c_ax = self.axes["offsets"][0][1].imshow(
                ...
                norm=matplotlib.colors.SymLogNorm(
                    linthresh=np.array(self.grid_shape[0].cpu() / 10),
                    vmin=0,
            ...
            c_ax = self.axes["offsets"][1][1].imshow(
                ...
                cmap=self.gray_cmp,
                norm=matplotlib.colors.SymLogNorm(
                    linthresh=np.array(self.grid_shape[1].cpu() / 10),
                    vmin=0,
    ...
    def calc_gt_flow(self, instance_mask, center_image, offset):
        ...
        for inst_id in instance_ids:
            ...
            if index_gt_center.sum() > 0:
                gt_center_position = (yxm_s[:, index_gt_center] - offset[:, index_gt_center] / self.grid_shape)
            else:
                gt_center_position = yxm_s[:, in_mask.squeeze()]
            gt_flow_mask[:, in_mask.squeeze()] = (yxm_s[:, in_mask.squeeze()] - gt_center_position)
        return gt_flow_mask
    ...
    def vis_segmentation(self, prediction, ground_truth, instance_mask_prev_frame):
        ...
        with torch.no_grad():
            ...
            pos_seg_pred = (torch.tanh(segm_prediction[0:2]) + self.grid[:, 0:height, 0:width].contiguous())
            ...
            grid = torch.stack((
                torch.arange(0, instance_mask.shape[0]).view(1, -1, 1).expand(1, *instance_mask.shape),
                torch.arange(0, instance_mask.shape[1]).view(1, 1, -1).expand(1, *instance_mask.shape),
            )).reshape(2, -1)

            segm_pred_clustered = self.cluster.cluster_pixels(segm_prediction, n_sigma=2)
            ...
            idx_seg_pred = idx_seg_pred[:, ~(idx_seg_pred[0] > instance_mask.shape[0] - 1)]

            grid = grid[:, ~(idx_seg_pred[1] > instance_mask.shape[1] - 1)]
            idx_seg_pred = idx_seg_pred[:, ~(idx_seg_pred[1] > instance_mask.shape[1] - 1)]

            idx_flat = np.ravel_multi_index((idx_seg_pred[0].cpu(), idx_seg_pred[1].cpu()), instance_mask.shape)
            grid_flat = np.ravel_multi_index((grid[0].cpu(), grid[1].cpu()), instance_mask.shape)
            centroid_pred = torch.zeros_like(instance_mask).flatten()  # visualize where the cell pix will be clustered
            ...
            # mask data
            instance_mask_masked = np.ma.array(instance_mask.cpu(), mask=instance_mask.cpu() == 0)
            segm_pred_clustered_masked = np.ma.array(segm_pred_clustered.cpu(), mask=segm_pred_clustered.cpu() == 0)
            ...
            if len(all_ids) > 0:
                min_id = min(all_ids)
                max_id = max(all_ids)
            self.axes["segmentation"][0].imshow(instance_mask_masked, cmap=self.gray_cmp, vmin=min_id, vmax=max_id)
            self.axes["segmentation"][0].set_title(r"GT Instances $t+1$")

            self.axes["segmentation"][1].imshow(segm_pred_clustered_masked, cmap=self.gray_cmp)
            self.axes["segmentation"][1].set_title(r"Pred. Instances $t+1$")

            # show original cell shape as well -> set as -1
            centroid_pred[torch.logical_and(instance_mask.detach() != 0, centroid_pred == 0)] = -10000
            centroid_pred_masked = np.ma.array(centroid_pred.cpu(), mask=centroid_pred.cpu() == 0)
            ids = np.unique(instance_mask.detach().cpu())
            ids = ids[ids > 0]

            self.axes["segmentation"][3].imshow(centroid_pred_masked, cmap=self.gray_cmp, vmin=min_id, vmax=max_id)
            self.axes["segmentation"][3].set_title(r"Offset Pred. $\mathbf{x}+\hat{\mathbf{o}}$ $t+1$")
            pixel_positions = np.unravel_index(idx_flat, instance_mask.shape)
            pixel_label = instance_mask.flatten().cpu()[grid_flat].numpy()
            self.add_centroid_marker(self.axes["segmentation"][3], pixel_positions, pixel_label, (min_id, max_id))

            self.axes["segmentation"][4].set_title(r"Center Prediction $t+1$")
            c_ax = self.axes["segmentation"][4].imshow(seed_map.cpu(), cmap=self.gray_cmp,
                                                       norm=mpl.colors.Normalize(vmin=0, vmax=1))
            self.add_colorbar(self.figures["segmentation"], c_ax, self.axes["segmentation"][4], ("seed_map", 4))

            self.axes["segmentation"][2].imshow(pix_pred.cpu().squeeze(), cmap=self.cmap_pix_pred)
            classes = [
            ...
            label = [mpl.patches.Patch(color=self.cmap_pix_pred(i), label=classes[i])
                     for i in range(self.cmap_pix_pred.N)]
            self.axes["segmentation"][2].set_title(r"Predicted Class vs GT")
            self.axes["segmentation"][2].legend(handles=label, title=r"GT$\rightarrow$Prediction", ncol=2,
                                                loc="lower left", fontsize=matplotlib.rcParams["font.size"])
            return self.axes
    ...
    def vis_tracking(self, prediction, ground_truth, instance_mask_prev_frame):
        ...
        with torch.no_grad():
            ...
            self.axes["tracking"][0][0].imshow(instance_mask_masked, cmap=self.gray_cmp, vmin=min_id, vmax=max_id)
            self.axes["tracking"][1][0].imshow(instance_mask_prev_frame_masked,
                                               cmap=self.gray_cmp, vmin=min_id, vmax=max_id)
            ...
            c_ax = self.axes["tracking"][0][1].imshow(
                ...
                cmap=self.gray_cmp,
                norm=matplotlib.colors.SymLogNorm(
                    linthresh=np.array(torch.norm(self.grid_shape.float()).cpu() / 10),
                    vmin=0,
                    vmax=np.array(torch.norm(self.grid_shape.float()).cpu()),
                    base=10,
                    linscale=3,
                ),
            )
    ...
    @staticmethod
    def calc_magnitude_and_phase(flow):
...
def calc_grid(grid_x, grid_y, pixel_x, pixel_y, device='cpu'):
    xm = torch.linspace(0, pixel_x, grid_x, device=device).view(1, 1, -1).expand(1, grid_y, grid_x)
    ym = torch.linspace(0, pixel_y, grid_y, device=device).view(1, -1, 1).expand(1, grid_y, grid_x)
