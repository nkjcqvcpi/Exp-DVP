...
class TwoDimensionalDataset(Dataset):
    ...
    def __init__(self, crop_dir="./", data_dir="./", data_subsets=[], center="center-medoid", type="train", bg_id=0,
                 size=None, transform=None, translation_prob=0.5, max_rel_translation=0.1):
        """
        ...
        print("2-D `{}` dataloader created! Accessing data from {}/{}/".format(type, crop_dir, data_subsets))
        ...
        for sub_set in data_subsets:
            img_list = glob.glob(os.path.join(crop_dir, "{}/".format(sub_set), "images/*.tif"))
            img_list.sort()
            image_list.extend(img_list)
            print("Number of images in `{}` directory is {}".format(sub_set, len(img_list)))

            inst_list = glob.glob(os.path.join(crop_dir, "{}/".format(sub_set), "masks/*.tif"))
            print("Number of instances in `{}` directory is {}".format(sub_set, len(inst_list)))
            inst_list.sort()
            instance_list.extend(inst_list)

            center_img_list = glob.glob(os.path.join(crop_dir, "{}/".format(sub_set), center + "/*.tif"))
            print("Number of center images in `{}` directory is {}".format(sub_set, len(center_img_list)))
            center_img_list.sort()
            center_image_list.extend(center_img_list)

            flow_img_list = glob.glob(os.path.join(crop_dir, "{}/".format(sub_set), center + "-flow" + "/*.tif"))
    ...
    def get_image_pairs(self):
        ...
        for i, path_img_file in enumerate(self.image_list):
            ...
            path_flow_img = os.path.join(os.path.dirname(path_img), "-".join([self.center, "flow"]), flow_img_name)

            try:
                pairs.append((self.image_list.index(os.path.join(path_img, name_next_img)), i,
    ...
    def get_sample(self, index, translated=False):
        """
        ...
        if translated:
            ...
            sub_dirs = path_parts[path_parts.index(data_set) + 1: path_parts.index("images")]
            image = tifffile.imread(os.path.join(self.data_dir, *sub_dirs, "images", full_img_name))
            center_image = tifffile.imread(os.path.join(self.data_dir, *sub_dirs, self.center, full_img_name))
            instance_image = tifffile.imread(os.path.join(self.data_dir, *sub_dirs, "masks", full_img_name))
            ...
            top_left = (np.random.randint(height - self.crop_size[0]), np.random.randint(width - self.crop_size[1]),)
            # calculation of the min max bounds of the offset
            translation = (
                np.random.randint(
                    -min(top_left[0], self.max_offset * self.crop_size[0]),
                    min(height - self.crop_size[0] - top_left[0] - 1, self.max_offset * self.crop_size[0]),
                ),
                np.random.randint(
                    -min(top_left[1], self.max_offset * self.crop_size[1]),
                    min(width - self.crop_size[1] - top_left[1], self.max_offset * self.crop_size[1]),
                ),
            )
            top_left_translated = (top_left[0] + translation[0], top_left[1] + translation[1])
        ...
        if len(self.instance_list) != 0:
            if not translated:
                instance_curr = tifffile.imread(self.instance_list[index_curr])  # YX or DYX (one-hot!)
                instance_prev = tifffile.imread(self.instance_list[index_prev])  # YX or DYX (one-hot!)
            ...
            instance_curr = self.convert_yx_to_cyx(instance_curr, key="instance")  # CYX or CDYX
            instance_prev = self.convert_yx_to_cyx(instance_prev, key="instance")  # CYX or CDYX
        ...
        if len(self.center_image_list) != 0:
            if translated:
                center_image_curr = self.convert_yx_to_cyx(center_image_curr, key="center_image")  # CYX
                center_image_prev = self.convert_yx_to_cyx(center_image_prev, key="center_image")  # CYX
    ...
    def get_image_crop(self, image, top_left):
        return image[top_left[0]: top_left[0] + self.crop_size[0], top_left[1]: top_left[1] + self.crop_size[1]]

    def __getitem__(self, index):
        if self.n_pairs == 1:
            index = 0
        else:
            index = (index if self.size is None else random.randint(0, len(self.pair_index) - 1))
        sample = self.get_sample(index, translated=np.random.rand() <= self.p_translation)
        # transform
...
def get_dataset(opts, typ, set_transforms):
    if opts.DATALOADER.NAME == "2d":
        return TwoDimensionalDataset(crop_dir=opts.DATALOADER.CROP_DIR, data_dir=opts.DATALOADER.DATA_DIR,
                                     data_subsets=opts.DATALOADER.TRAIN_SUBSETS if typ == 'train'
                                     else opts.DATALOADER.VAL_SUBSETS,
                                     center=opts.DATALOADER.CENTER, type=typ,
                                     size=opts.TRAIN.TRAIN_SIZE if typ == 'train' else opts.TRAIN.VAL_SIZE,
                                     transform=set_transforms)
    else:
        raise RuntimeError("Dataset {} not available".format(opts.DATALOADER.NAME))
