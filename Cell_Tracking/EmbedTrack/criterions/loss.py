...
class EmbedTrackLoss(nn.Module):
    def __init__(self, cluster, grid_y=1024, grid_x=1024, pixel_y=1, pixel_x=1, n_sigma=2, foreground_weight=1):
        """
        ...
        print(f"Created spatial emb loss function with: n_sigma: {n_sigma}, foreground_weight: {foreground_weight}")
        print("*************************")
    ...
    def forward(self, predictions, instances, labels, center_images, offsets, w_inst=1, w_var=10, w_seed=1,
                iou=False):
        """
        Args:
        ...
        # instances B 1 Y X
        batch_size, height, width = (segmentation_predictions.size(0), segmentation_predictions.size(2),
        ...
        track_loss, track_count = 0, 0

        iou_score = []

        loss_values = {"instance": 0, "variance": 0, "seed": 0, "track": 0}
        for b in range(0, batch_size):
            seed_loss_it, seed_loss_count = 0, 0
            ...
            sigma = torch.sigmoid(segmentation_predictions[b, 2 : 2 + self.n_sigma])  # n_sigma x h x w

            seed_map = torch.sigmoid(segmentation_predictions[b, 2 + self.n_sigma : 2 + self.n_sigma + 1])  # 1 x h x w
            # loss accumulators
            var_loss, instance_loss, seed_loss = 0, 0, 0
            ...
            if len(instance_ids) == 0:  # background image
                continue
            # n_dim; n_sigma
            all_sigmas = torch.stack([sigma[:, instance.eq(inst_id).squeeze()].mean(dim=1)
                                      for inst_id in instance_ids]).T
            ...
            for i, inst_id in enumerate(instance_ids):
                ...
                dist = torch.exp(- torch.sum(torch.pow(spatial_emb - center, 2) * s, 0, keepdim=True))

                # apply lovasz-hinge loss
                instance_loss = instance_loss + lovasz_hinge(dist * 2 - 1, in_mask.to(device))

                # seed loss
                seed_loss_it += self.foreground_weight * torch.mean(torch.pow(seed_map[in_mask] -
                                                                              dist[in_mask].detach(), 2))
                ...
                if b < (batch_size // 2):
                    ...
                    gt_prev_center_yxms = ((yxm_s[:, index_gt_center] - offsets[b, :, index_gt_center] / self.yx_shape)
                                           .view(-1, 1, 1).float())
                    dist_tracking = torch.exp(- torch.sum(torch.pow(tracking_emb - gt_prev_center_yxms, 2) * s, 0,
                                                          keepdim=True))
            ...
            if iou:
                instance_pred = self.cluster.cluster_pixels(segmentation_predictions[b], n_sigma=2, return_on_cpu=False)
                iou_scores = calc_iou_full_image(instances[b].detach(), instance_pred.detach(),)
                iou_score.append(torch.tensor(iou_scores, dtype=torch.float).mean())
            # seed_loss = seed_loss / torch.prod(torch.tensor(instances.shape[1:]))
            loss += w_inst * instance_loss + w_var * var_loss + w_seed * seed_loss
            loss_values["instance"] += (w_inst * instance_loss.detach() if isinstance(instance_loss, torch.Tensor)
                                        else torch.tensor(w_inst * instance_loss).float().to(device))
            loss_values["variance"] += (w_var * var_loss.detach() if isinstance(var_loss, torch.Tensor)
                                        else torch.tensor(w_var * var_loss).float().to(device))
            loss_values["seed"] += (w_seed * seed_loss.detach() if isinstance(seed_loss, torch.Tensor)
                                    else torch.tensor(w_seed * seed_loss).float().to(device))
            loss_values["track"] += (track_loss.detach() if isinstance(track_loss, torch.Tensor)
        ...
        if iou:
            return loss + segmentation_predictions.sum() * 0, loss_values, torch.tensor(iou_score).mean()
        else:
...
def calc_iou_full_image(gt, prediction):
    ...
    for gt_l in gt_labels:
        ...
        gt_ious = torch.tensor([calculate_iou(gt_mask, prediction.eq(p_l)) for p_l in overlapping_pred_labels])
        if len(gt_ious) > 0:
